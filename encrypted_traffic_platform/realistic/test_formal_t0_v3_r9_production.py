#!/usr/bin/env python3
"""Synthetic, NON_FORMAL tests for Formal T0 v3 R9 resume decisions."""
...
MODULE_PATH = Path(__file__).with_name("run_formal_t0_v3_r9_production.py")
...
class ProductionRetryV4Tests(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory(prefix="NON_FORMAL_R9_production_retry_")
    ...
    def fake_attempt(self, mode, number, selection, attempts_root, remote, **kwargs):
        ...
        self.assertIn("/formal/t0_v3_r9/", remote)
    ...
    def ledger(self):
        return formal.read_ledger(self.root / "formal_t0_v3_r9_retry_ledger.tsv")
    ...
    def exercise(self, failures, final_pass):
        ...
        with patch.object(self.retry, "run_attempt", side_effect=self.fake_attempt):
            if final_pass:
                ...
                self.assertEqual(metadata["dataset_track"], "realistic_t0_v3_r9")
    ...
    def test_atomic_ledger_replace_failure_preserves_previous(self):
        ledger = self.root / "formal_t0_v3_r9_retry_ledger.tsv"
...
class HotspotGovernanceTests(unittest.TestCase):
    ...
    def test_H4_safe_boundary_blocks_next_sample(self):
        ...
        with self.assertRaisesRegex(formal.HardStop,'R9_SAFE_BOUNDARY_HARD_STOP'):
...
class StorageGovernanceTests(unittest.TestCase):
    def setUp(self):
        self.temp=tempfile.TemporaryDirectory(prefix='NON_FORMAL_R9_storage_');self.addCleanup(self.temp.cleanup)
    ...
    def test_S6_insufficient_space_remains_blocked(self):
        with patch.object(formal,'free_disk_bytes',return_value=1),patch.object(formal,'dry_run') as full:
            with self.assertRaisesRegex(formal.HardStop,'R9_STORAGE_RECOVERY_INSUFFICIENT'):
                formal.storage_recover(FROZEN_HEAD)
...
staging = formal.load_staging_helper()


class ExecutorNamespaceTests(unittest.TestCase):
    def setUp(self):
        self.temp=tempfile.TemporaryDirectory(prefix='NON_FORMAL_R9_namespace_');self.addCleanup(self.temp.cleanup)
        self.root=Path(self.temp.name);self.source=self.root/'source-plan.json'
        self.source.write_bytes(b'{"immutable":true}\n');self.source.chmod(0o444)
        self.sha=staging.digest(self.source)
        self.req={'cache_root':str(self.root/'cache'),'revision':'t0_v3_r9','run_class':'FORMAL_PRODUCTION',
                  'run_id':'','sample_id':'formal_t0_v3_sample0001','attempt':1,'action':'prepare',
                  'inputs':{'workload_plan.json':self.sha}}

    def request(self,**overrides):return {**self.req,**overrides}
    def path(self,request):
        return staging.namespace_path(Path(request['cache_root']),request['revision'],request['run_class'],request['run_id'],request['sample_id'],request['attempt'])
    def prepared(self,request=None):
        request=request or self.req
        p=staging.handle(request);temp=Path(p['inputs']['workload_plan.json']['temporary_path'])
        temp.write_bytes(self.source.read_bytes())
        return request,p,temp
    def staged(self,request=None):
        req,p,tmp=self.prepared(request)
        result=staging.handle({**req,'action':'promote','temporary_paths':{'workload_plan.json':str(tmp)}})
        self.assertEqual(result['status'],'EXECUTOR_INPUT_PREP_PASS')
        return self.path(req)/'input/workload_plan.json'

    def test_N1_existing_validation_0444_formal_stages(self):
        validation=self.staged(self.request(run_class='NON_FORMAL_VALIDATION',run_id='quartet_one'))
        before=validation.read_bytes();target=self.staged()
        self.assertNotEqual(validation,target);self.assertEqual(validation.read_bytes(),before)
        self.assertEqual(validation.stat().st_mode&0o777,0o444)

    def test_N2_distinct_validation_formal_paths(self):
        self.assertNotEqual(self.path(self.req),self.path(self.request(run_class='NON_FORMAL_VALIDATION',run_id='one')))

    def test_N3_two_validation_runs_are_disjoint(self):
        a=self.staged(self.request(run_class='NON_FORMAL_VALIDATION',run_id='one'))
        b=self.staged(self.request(run_class='NON_FORMAL_VALIDATION',run_id='two'))
        self.assertNotEqual(a,b);self.assertFalse(self.path(self.req).exists())

    def test_N4_attempt1_attempt2_disjoint(self):
        self.assertNotEqual(self.staged(),self.staged(self.request(attempt=2)))

    def test_N5_sample1_sample2_disjoint(self):
        self.assertNotEqual(self.staged(),self.staged(self.request(sample_id='formal_t0_v3_sample0002')))

    def test_N6_existing_identical_reuse(self):
        target=self.staged();before=target.stat()
        result=staging.handle(self.req)
        self.assertEqual(result['inputs']['workload_plan.json']['status'],'REUSE_VERIFIED_IMMUTABLE_INPUT')
        self.assertEqual(target.stat().st_ino,before.st_ino);self.assertEqual(target.stat().st_mtime_ns,before.st_mtime_ns)

    def test_N7_existing_destination_mismatch_hard_stop(self):
        target=self.staged();request=self.request(inputs={'workload_plan.json':'a'*64})
        with self.assertRaisesRegex(staging.InputIntegrityError,'EXECUTOR_INPUT_INTEGRITY_MISMATCH'):
            staging.handle(request)
        self.assertEqual(target.read_bytes(),self.source.read_bytes())

    def test_N8_existing_0444_never_overwritten_or_chmodded(self):
        target=self.staged()
        with patch.object(Path,'chmod',side_effect=AssertionError('existing final chmod')), \
             patch.object(staging.os,'link',side_effect=AssertionError('existing final overwrite')):
            self.assertEqual(staging.handle(self.req)['status'],'EXECUTOR_INPUT_PREP_PASS')
        self.assertEqual(target.stat().st_mode&0o777,0o444)

    def test_N9_temp_verified_atomic_promotion(self):
        req,p,tmp=self.prepared();dest=self.path(req)/'input/workload_plan.json'
        self.assertFalse(dest.exists());self.assertTrue(tmp.name.startswith('.staging-'))
        original=staging.os.link;calls=[]
        def link(a,b):
            self.assertEqual(Path(a).stat().st_mode&0o777,0o444)
            self.assertEqual(staging.digest(Path(a)),self.sha);self.assertFalse(Path(b).exists())
            calls.append((a,b));return original(a,b)
        with patch.object(staging.os,'link',side_effect=link):
            staging.handle({**req,'action':'promote','temporary_paths':{'workload_plan.json':str(tmp)}})
        self.assertEqual(len(calls),1);self.assertEqual(dest.stat().st_mode&0o777,0o444)

    def test_N10_transferred_sha_mismatch_hard_stop(self):
        req,p,tmp=self.prepared();tmp.write_bytes(b'bad-transfer')
        with self.assertRaisesRegex(staging.InputIntegrityError,'transferred temporary SHA'):
            staging.handle({**req,'action':'promote','temporary_paths':{'workload_plan.json':str(tmp)}})
        self.assertFalse((self.path(req)/'input/workload_plan.json').exists())

    def test_N11_nonformal_not_formal_resume_source(self):
        self.staged(self.request(run_class='NON_FORMAL_VALIDATION',run_id='one'))
        self.assertEqual(staging.handle(self.request(action='inspect'))['status'],'INPUT_NOT_STAGED')
        with self.assertRaises(staging.InputIntegrityError):
            self.path(self.request(run_id='one'))

    def test_N12_fresh_R9_never_reads_R8_R7(self):
        for revision in ('t0_v3_r7','t0_v3_r8'):self.staged(self.request(revision=revision))
        self.assertEqual(staging.handle(self.request(action='inspect'))['status'],'INPUT_NOT_STAGED')
        self.staged()

    def test_source_and_final_permissions_0444_temp_writable(self):
        self.assertEqual(self.source.stat().st_mode&0o777,0o444)
        req,p,tmp=self.prepared();self.assertTrue(tmp.stat().st_mode&0o200)
        staging.handle({**req,'action':'promote','temporary_paths':{'workload_plan.json':str(tmp)}})
        self.assertEqual((self.path(req)/'input/workload_plan.json').stat().st_mode&0o777,0o444)

    def test_runner_reuse_does_not_scp(self):
        row_={**row(),'plan_path':str(self.source),'plan_sha256':self.sha}
        success={'status':'EXECUTOR_INPUT_PREP_PASS','inputs':{'workload_plan.json':{'status':'REUSE_VERIFIED_IMMUTABLE_INPUT'},formal.EXECUTOR.name:{'status':'REUSE_VERIFIED_IMMUTABLE_INPUT'}}}
        with patch.object(formal,'remote_staging',return_value=success), \
             patch.object(formal,'run_command',side_effect=AssertionError('unexpected scp overwrite')):
            formal.prepare_remote_inputs(None,row_,formal.executor_namespace(row_,1),1)

    def test_input_staging_failure_precedes_attempt_start(self):
        f=ProductionRetryV4Tests('test_policy_is_loaded_and_sha_enforced');f.setUp()
        try:
            with patch.object(formal,'prepare_remote_inputs',side_effect=formal.PrecheckFail('EXECUTOR_INPUT_INTEGRITY_MISMATCH')), \
                 patch.object(f.retry,'run_attempt',side_effect=AssertionError('browser/capture started')):
                with self.assertRaisesRegex(formal.PrecheckFail,'EXECUTOR_INPUT_INTEGRITY_MISMATCH'):
                    formal.run_formal_sample(f.schedule,FROZEN_HEAD,f.registry,1)
            self.assertFalse(list(f.root.rglob('attempt_1')))
        finally:f.doCleanups()


class ExecutorCrashTests(ExecutorNamespaceTests):
    # Select only crash methods in the suite below; shared fixtures retain the same staging logic.
    def test_C1_transfer_crash_before_promotion(self):
        req,p,old_temp=self.prepared()
        self.assertEqual(staging.handle(self.request(action='inspect'))['status'],'INPUT_NOT_STAGED')
        req,p,new_temp=self.prepared();self.assertNotEqual(old_temp,new_temp)
        staging.handle({**req,'action':'promote','temporary_paths':{'workload_plan.json':str(new_temp)}})
        self.assertTrue(old_temp.exists());self.assertEqual(staging.handle(self.request(action='inspect'))['status'],'EXECUTOR_INPUT_PREP_PASS')

    def test_C2_crash_after_atomic_promotion(self):
        req,p,tmp=self.prepared();original=staging.os.link
        def crash(a,b):original(a,b);raise OSError('after promotion crash')
        with patch.object(staging.os,'link',side_effect=crash):
            with self.assertRaisesRegex(OSError,'after promotion crash'):
                staging.handle({**req,'action':'promote','temporary_paths':{'workload_plan.json':str(tmp)}})
        result=staging.handle(req)
        self.assertEqual(result['inputs']['workload_plan.json']['status'],'REUSE_VERIFIED_IMMUTABLE_INPUT')

    def test_C3_partial_final_destination_rejected(self):
        req,p,tmp=self.prepared();target=self.path(req)/'input/workload_plan.json'
        target.write_bytes(b'partial');target.chmod(0o444)
        with self.assertRaisesRegex(staging.InputIntegrityError,'EXECUTOR_INPUT_INTEGRITY_MISMATCH'):
            staging.handle(req)
        self.assertEqual(target.read_bytes(),b'partial')

    def test_C4_old_temp_never_valid_input(self):
        req,p,tmp=self.prepared()
        self.assertTrue(tmp.exists())
        result=staging.handle(self.request(action='inspect'))
        self.assertEqual(result['status'],'INPUT_NOT_STAGED')
        self.assertFalse((self.path(req)/'input/workload_plan.json').exists())


if __name__=='__main__':
    groups=[('RETRY_RESUME', [ResumeDecisionTests,ProductionRetryV4Tests]),
            ('HOTSPOT',[HotspotGovernanceTests]),('STORAGE',[StorageGovernanceTests]),
            ('NAMESPACE',[ExecutorNamespaceTests]),('CRASH',[ExecutorCrashTests])]
    suite=unittest.TestSuite();counts={}
    for label,classes in groups:
        tests=unittest.TestSuite()
        for cls in classes:
            names=unittest.defaultTestLoader.getTestCaseNames(cls)
            if label=='CRASH':names=[n for n in names if n.startswith('test_C')]
            tests.addTests(cls(n) for n in names)
        counts[label]=tests.countTestCases();suite.addTest(tests)
    result=unittest.TextTestRunner(verbosity=2).run(suite)
    if not result.wasSuccessful():raise SystemExit(1)
    for label,n in counts.items():print(f'{label}_TESTS={n}/{n} PASS')
    print(f'TOTAL_PRODUCTION_TESTS={result.testsRun}/{result.testsRun} PASS')
    print('R9_EXECUTOR_NAMESPACE_TESTS_PASS')
    print('R9_PRODUCTION_GOVERNANCE_REGRESSION_PASS')
