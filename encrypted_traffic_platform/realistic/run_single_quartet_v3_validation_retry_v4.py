#!/usr/bin/env python3
"""Run a v3 validation quartet under frozen transient retry policy v4."""
...
POLICY = DOC / "formal_transient_retry_policy_v4.txt"
POLICY_FREEZE = DOC / "formal_transient_retry_policy_v4.sha256"
LEDGER = DOC / "formal_t0_v3_retry_v4_ledger.tsv"
POLICY_SHA256 = "48fb9290e9192d9068882693384ee24f37cc14c7bc40c9fdfe6e0d4b9f31d32a"
MAX_ATTEMPTS_PER_SAMPLE = 3
SUCCESS_MARKER = "REALISTIC_V3_SINGLE_QUARTET_VALIDATION_PASS"
EXPECTED_REDSOCKS_CONN_MAX = 256
def load_retryable_policy():
    import hashlib
    text = POLICY.read_text(encoding="utf-8")
    if hashlib.sha256(POLICY.read_bytes()).hexdigest() != POLICY_SHA256:
        raise RuntimeError("retry policy v4 SHA mismatch")
    if POLICY_FREEZE.read_text().split() != [POLICY_SHA256, POLICY.name]:
        raise RuntimeError("retry policy v4 freeze record mismatch")
    if "MAX_ATTEMPTS_PER_SAMPLE=3" not in text:
        raise RuntimeError("retry policy attempt limit mismatch")
    section = text.split("\nRETRYABLE_FAILURE_CLASSES\n", 1)[1].split("\nRETRY_AUTHORIZATION_GATES\n", 1)[0]
    return {line.split("=", 1)[0] for line in section.splitlines() if line.strip()}

RETRYABLE = load_retryable_policy()
...
def executor_initialization_health(started, stderr_text, sample_dir):
    """Read passive evidence after capture; unknown evidence denies initialization retry."""
    normalized = started.replace("T", " ").split(".", 1)[0] + " UTC"
    evidence = {"journals": {}}
    for host in (None, base.USER_HOST, base.SERVER_HOST):
        for kind, flags in (("kernel", ["-k"]), ("warning", ["-p", "warning"])):
            cmd = ["sudo", "-n", "journalctl", *flags, "--since", normalized, "--no-pager", "-o", "short-iso"]
            if host:
                cmd = ["ssh", "-o", "BatchMode=yes", host, shlex.join(cmd)]
            completed = base.run(cmd, 30)
            evidence["journals"][f"{host or 'collector'}_{kind}"] = {
                "returncode": completed.returncode, "stdout": completed.stdout, "stderr": completed.stderr}
    kernel = "\n".join(r["stdout"] for k, r in evidence["journals"].items() if k.endswith("kernel")).lower()
    warnings = "\n".join(r["stdout"] for k, r in evidence["journals"].items() if k.endswith("warning")).lower()
    combined = kernel + "\n" + warnings + "\n" + stderr_text.lower()
    patterns = {
        "oom": r"out of memory|oom-kill|oom_reaper",
        "kernel_kill": r"killed process|general protection fault",
        "segfault": r"segfault|segmentation fault|signal 11|sigsegv",
        "persistent_service_failure": r"main process exited|failed with result|core-dump|start request repeated too quickly",
        "system_resource_exhaustion": r"too many open files|cannot allocate memory|resource temporarily unavailable|no space left on device",
        "persistent_driver_failure": r"executable doesn't exist|cannot find module|error while loading shared libraries|permission denied",
    }
    evidence.update({key: int(bool(re.search(pattern, combined))) for key, pattern in patterns.items()})
    evidence["status"] = "PASS" if all(row["returncode"] == 0 for row in evidence["journals"].values()) and not any(evidence[key] for key in patterns) else "FAIL"
    base.write_json(sample_dir / "executor_initialization_health.json", evidence)
    return evidence
...
def run_attempt(
    ...
    try:
        ...
        mode_purity_report = classifier.audit_mode_purity(sample_dir, mode, base.tshark_count, workload=workload, phase_rows=classifier.read_phase_rows(sample_dir / "executor_phase.jsonl"))
        base.write_json(sample_dir / "mode_purity_report_v3.json", mode_purity_report)
        ...
        init_health = {}
        if failure["failure_class"] == classifier.DRIVER_CONNECTION_CLOSED:
            init_health = executor_initialization_health(mode_started, supervisor_stderr, sample_dir)
        ...
        result = {
            ...
            "workload_started": failure.get("workload_started"),
            "executor_initialization_health": init_health,
...
def resolve_attempt_failure(**kwargs):
    """Use classifier v3 precedence and the unchanged empty-response class."""
...
def retry_authorized(
    ...
    gates = {
        ...
        "mode_purity": attempt_result["mode_purity"] == ("NOT_EVALUABLE" if failure_class == classifier.DRIVER_CONNECTION_CLOSED else "PASS"),
    ...
    if failure_class == classifier.DRIVER_CONNECTION_CLOSED:
        evidence = attempt_result.get("executor_initialization_health", {})
        gates.update({
            "INITIALIZATION_STAGE": attempt_result.get("failure_stage") == "EXECUTOR_INITIALIZATION",
            "WORKLOAD_NOT_STARTED": attempt_result.get("workload_started") is False,
            "NO_TRAFFIC": attempt_result.get("packet_counts") == {"original": 0, "observed": 0, "egress": 0},
            "INITIALIZATION_HEALTH_EVIDENCE": evidence.get("status") == "PASS",
            "CAPACITY": attempt_result.get("mode") == "direct" or attempt_result.get("redsocks_actual_conn_max") == 256,
        })
        for name in ("oom", "kernel_kill", "segfault", "persistent_service_failure", "system_resource_exhaustion", "persistent_driver_failure"):
            gates[name] = evidence.get(name) == 0
...
def main() -> int:
    ...
    run_id = "single_quartet_v3_validation_retry_v4_" + base.stamp_now()
    ...
    try:
        ...
        if fatal_mode:
            summary = {
                "marker": "REALISTIC_V3_R5_SINGLE_QUARTET_FAIL",
                "classification": "NON_FORMAL_VALIDATION",
                "retry_policy": "FORMAL_T0_V3_TRANSIENT_RETRY_POLICY_V4",
            ...
            (run_root / "REALISTIC_V3_R5_SINGLE_QUARTET_FAIL").write_text(
                "REALISTIC_V3_R5_SINGLE_QUARTET_FAIL\nNON_FORMAL_VALIDATION\n", encoding="utf-8"
            )
            ...
            print("REALISTIC_V3_R5_SINGLE_QUARTET_FAIL")
            return 1
        ...
        print(f"REALISTIC_V3_R5_SINGLE_QUARTET_FAIL root={run_root} error={type(exc).__name__}: {exc}", flush=True)
        return 1
    finally:
        base.cleanup_mode()
        if not success:
