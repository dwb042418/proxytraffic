#!/usr/bin/env python3
"""Formal T0 v3 R8 schedule orchestrator.
...
ORCHESTRATOR_VERSION = "formal-t0-v3-r8-production-v1"
DATASET_TRACK = "realistic_t0_v3_r8"
ARTIFACT_CLASS = "FORMAL_PRODUCTION"
FORMAL_CLASSIFICATION = "FORMAL_T0_V3_R8_PRODUCTION"

REPO = Path("/home/etip/Tunnel/proxytraffic")
DOC = REPO / "docs/realistic_v1/formal_t0_v3"
LOCAL_PRODUCTION_ROOT = Path("/home/etip/datasets/staging/realistic_v1/t0_v3_r8")
REMOTE_PRODUCTION_ROOT = Path("/home/dataset-assist-0/duwenbiao/Tunnel/proxydata/realistic_v1/t0_v3_r8")
...
ROOT_AMENDMENT = DOC / "formal_t0_v3_r8_production_root_amendment.txt"
...
FROZEN_SHA256 = {
    ...
    ROOT_AMENDMENT: "21908fd1592ab0ba52667e4001500a5e80d066ccb98b0a3a11064fb9cd6a2401",
...
def verify_root_amendment() -> None:
    ...
    expected = {
        ...
        "scope": "FORMAL_T0_V3_R8_PRODUCTION_ONLY",
...
def configure_execution_components():
    spec = importlib.util.spec_from_file_location("formal_t0_v3_r8_retry_component", RETRY_RUNNER)
...
def scan_resume_state(
    ...
    ledger = read_ledger(local_root / "formal_t0_v3_r8_retry_ledger.tsv")
...
def run_formal_sample(
    ...
    remote_exec_root = f"/home/etip/.cache/proxytraffic-realistic-v3-formal-t0-r8/{sample_id}"
    prepare_remote_inputs(retry, row, remote_exec_root)
    selection = selection_from_row(row)
    ledger_path = LOCAL_PRODUCTION_ROOT / "formal_t0_v3_r8_retry_ledger.tsv"
...
def progress_from_schedule(
    ...
    state = scan_resume_state(schedule, LOCAL_PRODUCTION_ROOT, frozen_git_head)
    ledger = read_ledger(LOCAL_PRODUCTION_ROOT / "formal_t0_v3_r8_retry_ledger.tsv")
    attempts = len(ledger)
...
def write_progress(schedule: list[dict[str, str]], frozen_git_head: str) -> dict[str, object]:
    ...
    atomic_write_json(LOCAL_PRODUCTION_ROOT / "formal_t0_v3_r8_progress.json", progress)
    return progress
...
def write_hard_stop(reason: str, frozen_git_head: str) -> None:
    atomic_write_json(LOCAL_PRODUCTION_ROOT / "formal_t0_v3_r8_hard_stop.json", {
...
def monitor_path(root: Path | None = None) -> Path:
    return (root or LOCAL_PRODUCTION_ROOT) / "formal_t0_v3_r8_monitor_state.json"
...
def verify_hotspot_baseline() -> list[dict[str, str]]:
    ...
    for row in rows:
        if row["infra_clean"] != "true" or row["run_class"] not in {"R7_VALIDATION", "R8_VALIDATION"}:
...
def hotspot_evidence(frozen_git_head: str) -> list[dict[str, object]]:
    ...
    for entry in ledger:
        ...
        evidence.append({
            ...
            "run_class": "R8_VALIDATION" if OPERATIONAL else "R8_PRODUCTION",
...
def update_hotspot_monitor(row: dict[str, str], frozen_git_head: str) -> None:
    ...
    print("R8_PRODUCTION_RECURRENT_RETRY_HOTSPOT_REVIEW_REQUIRED", flush=True)
...
def mark_monitor_boundary(row: dict[str, str], result: dict[str, object], *, natural_hard_stop: bool = False) -> None:
    ...
    if not natural_hard_stop:
        raise HardStop("R8_SAFE_BOUNDARY_HARD_STOP" if safe else "REVIEW_REQUIRED: boundary cleanup not verified")


def eviction_ledger_path(root: Path | None = None) -> Path:
    return (root or LOCAL_PRODUCTION_ROOT) / "formal_t0_v3_r8_eviction_ledger.json"
...
def read_evictions(root: Path | None = None) -> dict[str, dict[str, object]]:
    ...
    if value.get("schema") != "R8_EVICTION_V1" or not isinstance(value.get("samples"), dict):
...
def perform_eviction(row: dict[str, str], frozen_git_head: str) -> dict[str, object]:
    ...
    atomic_write_json(eviction_ledger_path(), {"schema": "R8_EVICTION_V1", "samples": transactions})
    sample = LOCAL_PRODUCTION_ROOT / "samples" / row["schedule_id"]
...
def storage_recover(frozen_git_head: str) -> None:
    ...
    for row in schedule:
        ...
        if sample.exists() and any(sample.rglob("*.pcap")):
            perform_eviction(row, frozen_git_head)
    if free_disk_bytes() < MIN_FREE_BYTES:
        raise HardStop("R8_STORAGE_RECOVERY_INSUFFICIENT")
    dry_run(frozen_git_head)
    print("R8_STORAGE_RECOVERY_PASS", flush=True)
...
def operational_quartet(frozen_git_head: str) -> int:
    ...
    global FORMAL_CLASSIFICATION, OPERATIONAL, OPERATIONAL_CODE_SHA
    # Dedicated explicit NON-FORMAL entry, never configurable to a Formal root.
    OPERATIONAL = True
    OPERATIONAL_CODE_SHA = sha256(Path(__file__))
    stamp = safe_stamp()
    LOCAL_PRODUCTION_ROOT = Path("/home/etip/datasets/staging/realistic_v1/r8_operational_quartet") / stamp
    REMOTE_PRODUCTION_ROOT = Path("/home/dataset-assist-0/duwenbiao/Tunnel/proxydata/realistic_v1/r8_operational_quartet") / stamp
    DATASET_TRACK = "realistic_t0_v3_r8_operational_non_formal"
    ARTIFACT_CLASS = "NON_FORMAL_VALIDATION"
    FORMAL_CLASSIFICATION = "R8_NON_FORMAL_OPERATIONAL_QUARTET"
    ...
    print("R8_OPERATIONAL_QUARTET_PASS", flush=True)
    return code
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.dry_run:
            ...
            print("FORMAL_T0_V3_R8_PRODUCTION_DRY_RUN_PASS")
            return 0
        ...
        if not args.dry_run:
            ...
            try:
                ...
                pass
        print(f"FORMAL_T0_V3_R8_HARD_STOP: {type(exc).__name__}: {exc}", file=sys.stderr)
        return 1
