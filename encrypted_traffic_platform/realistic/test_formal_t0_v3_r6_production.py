#!/usr/bin/env python3
"""Synthetic, NON_FORMAL tests for Formal T0 v3 R6 resume decisions."""
...
MODULE_PATH = Path(__file__).with_name("run_formal_t0_v3_r6_production.py")
...
class ProductionRetryV4Tests(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory(prefix="NON_FORMAL_R6_production_retry_")
    ...
    def fake_attempt(self, mode, number, selection, attempts_root, remote, **kwargs):
        ...
        self.assertIn("formal-t0-r6/", remote)
    ...
    def ledger(self):
        return formal.read_ledger(self.root / "formal_t0_v3_r6_retry_ledger.tsv")
    ...
    def exercise(self, failures, final_pass):
        ...
        with patch.object(self.retry, "run_attempt", side_effect=self.fake_attempt):
            if final_pass:
                ...
                self.assertEqual(metadata["dataset_track"], "realistic_t0_v3_r6")
    ...
    def test_atomic_ledger_replace_failure_preserves_previous(self):
        ledger = self.root / "formal_t0_v3_r6_retry_ledger.tsv"
...
if __name__ == "__main__":
    ...
    print("R6_PRODUCTION_RETRY_V4_SYNTHETIC_PASS")
